...
class Daft:
    ...
    def search(self, latitude: float, longitude: float, radius: float, max_pages: Optional[int] = None) -> List[Listing]:

        _payload = self._make_payload()

        min_latitude = latitude - (radius / 111)
        max_latitude = latitude + (radius / 111)
        min_longitude = longitude - (radius / (111 * math.cos(math.radians(latitude))))
        max_longitude = longitude + (radius / (111 * math.cos(math.radians(latitude))))

        _payload["filters"]["latitude"] = {"min": min_latitude, "max": max_latitude}
        _payload["filters"]["longitude"] = {"min": min_longitude, "max": max_longitude}
